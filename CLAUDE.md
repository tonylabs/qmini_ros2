...
| **M4** | Calibration | 🔄 | `qmini_calibration`: **IMU noise/mount** (`imu_noise_calib`, ran on real N100 — noise GREEN vs DR, \|g\|=9.84) **and bus-jitter** (`bus_jitter_calib`: driver-stamped `/joint_states` rate/jitter/p99/dropped-ticks + per-channel NaN, diff vs live `sim.dt`/`decimation`) measurements done. Bus-jitter logic validated on synthetic; **run on the Pi for real numbers**. Each has launch+bag+analyzer; `diff_against_isaaclab.py` is the single-source diff. **Remaining:** actuator latency + effective PD, friction; IMU mount-tilt check needs the base leveled | `src/qmini_calibration/README.md` |
| **M5** | Policy runner | 🔄 | `qmini_rl/policy_runner_node` **implemented**: ONNX Runtime C++ loads the Isaac-Lab policy (`obs[44]→action[10]`), assembles the 44-dim observation on-robot (per-term scales, `gait_phase_sincos` `[sinL,sinR,cosL,cosR]` @1.5 Hz, `static_flag` @0.15), publishes `q_des=home+0.5·a` on `/joint_target` at 50 Hz, MotionGate-gated. Builds + validated on x86 with synthetic inputs. **Pending: verify joint order via Isaac Lab `play.py`, run on real robot (Pi aarch64 onnxruntime + real IMU/joint_states).** Depends on `qmini_imu` | `src/qmini_rl/README.md` |
| **—** | IMU driver | ✅ | `qmini_imu` (C++, native FDILink parser over raw termios, no external serial dep): Wheeltek N100 → `sensor_msgs/Imu` on `/imu/data`, own process, graceful port-loss retry. **Validated on real N100 2026-05-26**: parses CRC-checked frames, gravity \|a\|≈9.83 m/s², ~65 Hz (>50 Hz policy rate). Frame alignment to `base_link` still confirmed/tuned in M4 | `src/qmini_imu/README.md` |
...
**Immediate next action:** M2 closed; `qmini_imu` hardware-validated; `qmini_rl` policy runner implemented + validated on x86 with synthetic inputs; M4 IMU noise/mount measurement done (noise floors GREEN vs DR). Before a real-robot policy run: (1) **verify `kJointOrder`** against Isaac Lab `play.py` `env.scene["robot"].joint_names` (PhysX DOF order is inferred, not stored), (2) build with the **aarch64 onnxruntime** on the Pi (vendored in `qmini_official_sdk`), (3) finish M4 (actuator latency/effective PD, friction) and run the **mount-tilt check with the base leveled** to confirm the IMU→`base_link` frame `imu_projected_gravity` depends on. IMU-noise + bus-jitter measurements are built; the next M4 build is the **actuator-latency + effective-PD** measurement (needs the motor bus under low-gain torque).
