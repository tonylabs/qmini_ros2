...
**Immediate next action:** M2 closed; `qmini_imu` hardware-validated; `qmini_rl` policy runner implemented + validated on x86 with synthetic inputs. Before a real-robot policy run: (1) **verify `kJointOrder`** against Isaac Lab `play.py` `env.scene["robot"].joint_names` (PhysX DOF order is inferred, not stored), (2) build with the **aarch64 onnxruntime** on the Pi (vendored in `qmini_official_sdk`), (3) **M4 calibration** to confirm the IMU→`base_link` frame alignment that `imu_projected_gravity` depends on. M4 is now the gating prerequisite for trusting the policy on hardware.
