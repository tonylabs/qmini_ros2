...
**Immediate next action:** M2 closed and the `qmini_imu` driver is built + hardware-validated. The natural path is now **M4 calibration → M5 policy runner**. The IMU driver unblocks both the M4 IMU-noise/mount measurements (and the rest of the M4 sweep) and the M5 44-dim observation assembler. M4 is also where the IMU frame alignment to `base_link` (the Isaac Lab `ImuCfg` mount) gets confirmed.
