...
| **M4** | Calibration | 🔄 | `qmini_calibration`: **IMU noise/mount measurement done** (`imu_noise_calib` node + launch+bag, `analyze_imu_bag.py`, `diff_against_isaaclab.py` single-source diff). Ran on real N100 2026-05-26: gyro/proj-grav noise far inside DR (GREEN), \|g\|=9.84. **Remaining:** bus jitter, actuator latency + effective PD, friction; and the IMU mount-tilt check needs the base leveled | `src/qmini_calibration/README.md` |
